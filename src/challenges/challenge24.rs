...
pub fn challengetwentyfour() -> Result<(), Box<dyn std::error::Error>> {
    let output = solve(include_str!("../../assets/challenge24/input.txt"), false);
    println!("The output of the first part is {}", output);
    let output = solve(include_str!("../../assets/challenge24/input.txt"), true);
...
fn solve(input: &str, smallest: bool) -> i64 {
    ...
    for line in input.trim().split('\n') {
        ...
        program.push(match (instruction, reg, b_value) {
            ...
            ("eql", a, Some(b)) => Instruction::Eq(a, b),
            _ => {
                panic!();
            }
        });
