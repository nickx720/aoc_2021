...
use std::time;
pub fn challengetwentytwo() -> Result<(), Box<dyn std::error::Error>> {
    let input: String = read_to_string("assets/challenge22/input.txt")?;
    let steps = parse_input(&input);
    let grid_root = create_tree(&steps);
    let answer = part1(&grid_root);
    println!("The output of the first part is {}", answer);
    let answer = part2(&grid_root);
    println!("The output of the second part is {}", answer);
    Ok(())
}
...
mod tests {
    ...
    use super::*;
    #[test]
    fn part_one_solve() -> Result<(), Box<dyn std::error::Error>> {
        let input: String = read_to_string("assets/challenge22/sample.txt")?;
        let steps = parse_input(&input);
        let grid_root = create_tree(&steps);
        let answer = part1(&grid_root);
    ...
    #[test]
    fn part_two_solve() -> Result<(), Box<dyn std::error::Error>> {
        let input: String = read_to_string("assets/challenge22/sample.txt")?;
        let steps = parse_input(&input);
        let grid_root = create_tree(&steps);
        let answer = part2(&grid_root);
        assert_eq!(answer, 39769202357779);
        Ok(())
    }
}
